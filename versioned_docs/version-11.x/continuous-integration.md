...
install:
  ...
  - ps: $env:PNPM_VERSION = "latest-11"; Invoke-WebRequest https://get.pnpm.io/install.ps1 -UseBasicParsing | Invoke-Expression
  - ps: pnpm install
```
...
requirements](./installation.md#on-posix-systems).
...
pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                sh 'curl -fsSL https://get.pnpm.io/install.sh | env PNPM_VERSION=latest-11 sh -'
                sh 'pnpm install'
            }
        }
    }
