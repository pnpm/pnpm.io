...
`latest` on npm still points at the pnpm 11 line, so install pnpm 12 from the `latest-12` tag:

```sh
pnpm self-update latest-12
```

See [Installing pnpm 12](/installation) for the other ways, including without Node.js. Homebrew, winget, Scoop, and Chocolatey don't offer it yet.
