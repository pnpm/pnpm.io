...
- **Supply-chain protection on by default.** [`minimumReleaseAge`](/settings/dependency-resolution#minimumreleaseage) defaults to `1440` (1 day) and [`blockExoticSubdeps`](/settings/dependency-resolution#blockexoticsubdeps) defaults to `true`.
...
- **Native publish flow.** [`pnpm publish`](/cli/publish), [`login`](/cli/login), [`logout`](/cli/logout), [`view`](/cli/view), [`deprecate`](/cli/deprecate), [`unpublish`](/cli/unpublish), [`dist-tag`](/cli/dist-tag), and [`version`](/cli/version) no longer delegate to the npm CLI.
...
`onlyBuiltDependencies`, `onlyBuiltDependenciesFile`, `neverBuiltDependencies`, `ignoredBuiltDependencies`, and `ignoreDepScripts` have all been removed. Use [`allowBuilds`](/settings/build#allowbuilds) instead — a map from package name patterns to booleans:
...
Reimplemented: [`publish`](/cli/publish), [`view`](/cli/view) (`info`, `show`, `v`), [`login`](/cli/login) (`adduser`), [`logout`](/cli/logout), [`deprecate`](/cli/deprecate), [`unpublish`](/cli/unpublish), [`dist-tag`](/cli/dist-tag), [`version`](/cli/version), [`search`](/cli/search), [`star`/`unstar`/`stars`](/cli/star), [`whoami`](/cli/whoami), [`ping`](/cli/ping), [`docs`/`home`](/cli/docs).
...
| --- | --- |
| [`pnpm ci`](/cli/ci) | Runs `pnpm clean` followed by `pnpm install --frozen-lockfile`. Aliases: `clean-install`, `ic`, `install-clean`. |
| [`pnpm clean`](/cli/clean) | Removes `node_modules` from all workspace projects. `--lockfile` also removes `pnpm-lock.yaml`. |
| [`pnpm sbom`](/cli/sbom) | Generates a Software Bill of Materials in CycloneDX 1.7 or SPDX 2.3 JSON. |
| [`pnpm peers check`](/cli/peers) | Reports unmet/missing peers from the lockfile. |
| [`pnpm runtime set`](/cli/runtime) | Installs a runtime; deprecates `pnpm env use`. |
| [`pnpm docs`](/cli/docs) / `home` | Opens the package homepage. |
| [`pnpm ping`](/cli/ping) | Pings the registry. |
| [`pnpm with`](/cli/with) | Runs pnpm at a specific (or current) version for a single invocation, bypassing `packageManager` pins. |
| [`pnpm pack-app`](/cli/pack-app) | Packs a CommonJS entry into a standalone executable for one or more target platforms via [Node.js SEA](https://nodejs.org/api/single-executable-applications.html). |

Plus the natively reimplemented commands listed under "Native publish flow" above, and short aliases [`pn`](/cli/pn) for `pnpm` and [`pnx`](/cli/pnx) for `pnpx`/`pnpm dlx`.
