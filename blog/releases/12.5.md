...
or `pypi`. Python searches indexes in declaration order and uses the first that
contains the package. Cargo accepts one sparse index. Credentials come from
`.npmrc`, matched by origin; registry URL keys cannot contain credentials.
