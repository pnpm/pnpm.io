...
[`pnpm self-update`](/cli/self-update#installing-pnpm-v12) and `packageManager` version-switching can now install and link pnpm v12, the Rust port, published under both the `pnpm` and `@pnpm/exe` names on the `next-12` dist-tag:
