...
is untouched. `pnpm cache view` now shows full registry URLs. `pnpm cache list` and
`pnpm cache list-registries` print the new encoded directory names in v12.4.x,
so scripts that parse them need updating. Since v12.5.0,
`pnpm cache list-registries` prints full URLs; `pnpm cache list` continues to
print encoded directory names.
