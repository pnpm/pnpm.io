...
A new [`trustLockfile`](/settings/dependency-resolution#trustlockfile) setting controls whether `pnpm install` re-applies the [`minimumReleaseAge`](/settings/dependency-resolution#minimumreleaseage) / `trustPolicy: 'no-downgrade'` checks to every entry in the loaded lockfile. When `true`, the install treats the lockfile as already-trusted and skips the verification pass — useful for closed-source projects where every commit comes from a trusted author. The default is `false`, so verification stays on by default.
