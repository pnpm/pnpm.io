...
The new [`frozenStore`](/settings/store#frozenstore) setting (`--frozen-store`) lets `pnpm install` run against a package store that lives on a **read-only filesystem** — a [Nix](https://nixos.org/) store, a read-only bind mount, or an OCI image layer.
...
The store must already contain everything the install needs, including the build output of any package whose lifecycle scripts are approved (or that has a patch). If a required build is missing under the [global virtual store](/settings/node-modules#enableglobalvirtualstore), the install fails up front with `ERR_PNPM_FROZEN_STORE_NEEDS_BUILD` instead of crashing mid-build on a read-only write — so seed those builds first.
