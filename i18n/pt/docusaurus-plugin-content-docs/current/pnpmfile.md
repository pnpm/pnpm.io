...
pnpm permite entrar diretamente no processo de instalação através de funções especiais chamadas (hooks). Hooks can be declared in a file called `.pnpmfile.mjs` (ESM) or `.pnpmfile.cjs` (CommonJS).

By default, `.pnpmfile.mjs` should be located in the same directory as the lockfile. For instance, in a [workspace](workspaces.md) with a shared lockfile, `.pnpmfile.mjs` should be in the root of the monorepo.
