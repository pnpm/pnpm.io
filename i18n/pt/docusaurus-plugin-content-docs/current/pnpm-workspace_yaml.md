...
O arquivo `pnpm-workspace.yaml` define a raiz do workspace [][] e possibilita que você inclua / exclua diretórios do workspace. If the `packages` field is omitted, only the root package is included in the workspace.
...
O pacote raiz é sempre incluído, mesmo quando os curingas de localização customizado são usados.
