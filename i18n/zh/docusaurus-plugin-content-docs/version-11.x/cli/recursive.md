...
请注意，我们鼓励使用 [`pnpm-workspace.yaml`] 代替来进行此设置，去强制所有环境中都有同样的表现。 此选项单独存在，以便你可以在必要时覆盖它。
...
即使使用了 `--no-bail` ，所有任务也都将完成，除非任意的任务执行失败，那么该命令将以非零码（non-zero code）退出。
