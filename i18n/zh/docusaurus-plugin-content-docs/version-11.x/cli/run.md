...
你现在可以使用 `pnpm run watch` 运行该脚本！ 很简单吧？
对于那些不喜欢敲键盘而浪费时间的人要注意的另一件事是，所有脚本都会有 pnpm 命令的别名，所以最终 `pnpm run watch` 的简写是 `pnpm watch` （<strong>仅适用于</strong>那些不与已有的 pnpm 命令相同名字的脚本）。
...
所以如果 `watch` 运行 `webpack --watch`，那么这个命令：
...
并输出前缀流。 对于许多包中长时间运行的进程，这是首选标志，例如漫长的构建进程。
...
从并行运行的子进程的输出中隐藏工作空间前缀，并且仅打印原始输出。 如果你在 CI 上运行并且输出必须采用特定格式而没有任何前缀（例如 [GitHub Actions 注释](https://docs.github.com/en/actions/using-workflows/workflow-commands-for-github-actions#setting-an-error-message)），则此功能很有用。 仅支持 `--reporter=append-only`。
