...
另一种降低安装受损软件包风险的方法是推迟更新依赖包。 由于恶意软件通常会很快被检测到，因此将更新延迟 24 小时很可能会阻止你安装错误版本。 [minimumReleaseAge] 设置定义了版本发布后 pnpm 安装之前必须经过的最少分钟数。 在 pnpm v11 中，默认值为 `1440`（1 天），这意味着新发布的软件包至少要 1 天后才能被解析。 若要选择退出，在 `pnpm-workspace.yaml` 中设置 `minimumReleaseAge: 0`。 你也可以将其设置为 `10080`，以便在安装新版本之前等待一周。

### 强制执行信任政策

为了进一步保护你的供应链，pnpm 还支持 [trustPolicy] 设置。pnpm 当设置为 `no-downgrade` 时，如果软件包的信任级别与以前的版本相比有所降低（例如，如果该软件包以前是由受信任的发布者发布的，但现在只有来源信息或没有信任证据），则此设置将阻止安装该软件包。 这可以帮助你避免安装可能已被入侵或不太可信的版本。
