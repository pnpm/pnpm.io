...
![pnpm 内容可寻址存储的示意图。 在插图中有两个带有 node_modules 的项目。 node_modules 目录下的文件被硬链接至内容可寻址存储的相同文件处。](/img/pnpm-store.svg)
...
![pnpm安装过程的示意图。 包将被以最快速度解析、获取，并硬链接。](/img/installation-stages-of-pnpm.svg)

这种方法比传统的三阶段安装过程（解析、获取和写入所有依赖项到 `node_modules`）要快得多。

![Yarn Classic 或 npm 等包管理器如何安装依赖项的说明。](/img/installation-stages-of-other-pms.svg)
...
- [扁平化的 node_modules 并非唯一方法](/blog/2020/05/27/flat-node-modules-is-not-the-only-way)
- [符号链接的 node_modules 结构](symlinked-node-modules-structure.md)

:::tip

如果你的工具不适用于符号链接，你仍然可以使用 pnpm 并将 [node-linker](settings#nodeLinker) 设置设置为 `hoisted`。 这样 pnpm 就会创建一个类似于 npm 和 Yarn Classic 创建的 node_modules 目录。
