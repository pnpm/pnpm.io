...
如果你正在开发包含二进制文件的包，例如命令行工具，你可以使用 `pnpm link` 使二进制文件在整个系统范围内可用。
这与使用 `pnpm install -g foo` 相同，但它将使用 `foo` 的本地版本，而不是从注册源下载。
...
在处理 **对等依赖** 时，建议使用 `file:` 协议。 它更好地解决了项目依赖项中的对等依赖项，确保链接的依赖项正确使用主项目中指定的依赖项版本，从而实现更加一致和预期的行为。
