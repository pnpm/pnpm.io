...
`.changeset` 目录中生成的 markdown 文件应被提交到存储库。
...
要自动执行此过程，你可以将 `changeset version` 与 GitHub actions 一起使用。 当检测到 changeset 文件被合并到 main 分支时，该 action 将创建一个新的 PR，列出所有应该变更版本号的包。 每次有新的变更集文件到达 main 时，PR 都会自动更新。 一旦合并，包将被更新，并且如果在 Action 中指定了 `publish` 输入，则它们将使用给定的命令发布。
...
在 `.github/workflows/changesets.yml` 中添加新的工作流程。 此工作流程将创建一个新的分支和 PR，因此应在存储库设置 (`github.com/<repo-owner>/<repo-name>/settings/actions`) 中授予 Action 读取和写入权限。 如果 `changesets/action` 步骤上包含 `publish` 输入，存储库还应该包含一个 npm 认证令牌作为仓库密钥，名为 `NPM_TOKEN`。
...
关于 changesets Action 的更多信息和文档可以在
[这里](https://github.com/changesets/action) 找到。
