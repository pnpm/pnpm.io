...
使用 `--save-peer` 将添加一个或多个软件包到 `peerDependencies` 并将它们作为开发依赖项安装。

### --save-catalog

添加于：v10.12.1

将新依赖项保存到默认的 [目录（catalog）][].

### --save-catalog-name &lt;catalog_name\>

添加于：v10.12.1

将新的依赖项保存到指定的 [目录（catalog）][]。

### --config

添加于：v10.8.0

将依赖项保存到 [配置依赖项（configDependencies）](config-dependencies.md)。
...
这将运行 `esbuild`的 postinstall 脚本，并将其添加到 `pnpm-workspace.yaml` 的 ` allowBuilds ` 字段。 因此， `esbuild` 将来将始终被允许运行其脚本。
