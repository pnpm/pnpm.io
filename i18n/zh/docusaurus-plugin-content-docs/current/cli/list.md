...
如果位置参数是 `name-pattern@version-range` 标识符，会将结果限制为仅名称相符的包。 例如，`pnpm list "babel-*" "eslint-*" semver@5`。
...
仅仅显示在`dependencies` 和 `optionalDependencies`中的 ` package `的依赖关系图。

### --dev, -D

仅仅显示在` devDependencies `中的 ` package `的依赖关系图。

### --no-optional

显示除 `optionalDependencies` 之外的 ` package `的依赖关系图。
