...
当设置了 [`enableGlobalVirtualStore`](../settings.md#enableglobalvirtualstore) 选项时， `pnpm deploy` 会忽略它，并始终在部署目录中创建一个本地虚拟存储。 这样可以保持部署目录的自包含性和可移植性。
...
如果你在部署之前构建过项目，也可以使用 `--prod` 选项跳过 `devDependencies` 安装。
...
默认情况下，项目的所有文件会在部署期间被复制，但这可以通过以下 _之一_ 进行修改，并按顺序解决：
