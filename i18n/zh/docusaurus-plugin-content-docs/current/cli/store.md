...
当启用 [全局虚拟存储][] 时， `pnpm store prune` 还会对全局虚拟存储的 `links/` 目录执行标记清除垃圾回收。 使用该存储的项目通过符号链接在 `{storeDir}/v11/projects/`中注册，允许 pnpm 跟踪活跃使用情况并安全地从全局虚拟存储中删除未使用的软件包。
...
[全局虚拟存储]: ../settings.md#enableglobalvirtualstore
