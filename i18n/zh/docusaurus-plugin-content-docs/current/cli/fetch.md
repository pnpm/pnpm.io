...
只要不更改 `.npmrc`， `package.json`， `pnpm-lock.yaml`，`pnpm-workspace.yaml`, `.pnpmfile.mjs`， Docker 构建缓存一直有效至 `RUN pnpm install --frozen-lockfile --prod` 层，而其在构建 Docker 镜像时耗费绝大部分时间。
...
[官方指南]: https://github.com/nodejs/docker-node#readme
