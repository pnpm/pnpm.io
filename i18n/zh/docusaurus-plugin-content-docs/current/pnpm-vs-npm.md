...
从 v3 开始，npm 使用[平铺的依赖结构][]。 这可以减少磁盘空间占用， 但却导致 `node_modules` 目录的混乱。

另一方面，pnpm 通过使用硬链接和符号链接到全局磁盘内容可寻址存储来管理 `node_modules`。 这将给你带来减少磁盘空间使用的好处，同时也能保持你的`node_modules`整洁。 如果你希望了解更多信息，可以参考[存储布局][]。
...
如果你需要同时在多个包中运行安装，例如在单仓库的情况下，您应该查看 [`pnpm -r`][] 的文档。

[平铺的依赖结构]: https://github.com/npm/npm/issues/6912
[存储布局]: symlinked-node-modules-structure
[有助于避免愚蠢的错误]: https://www.kochan.io/nodejs/pnpms-strictness-helps-to-avoid-silly-bugs.html
