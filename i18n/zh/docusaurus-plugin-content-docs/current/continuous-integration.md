...
在 [AppVeyor][]上，你可以使用 pnpm 安装依赖项，只需将此 添加到你的 `appveyor.yml`中即可：
...
在 [Semapore][]上 ，将此内容添加到 `.semaphore/semaphore.yml` 文件中以使用 pnpm 来安装和缓存你的依赖：
...
在 [Travis CI][]上，你可以使用 pnpm 安装依赖项，方法是将此 添加到你的 `.travis.yml` 文件中：
...
[Semapore]: https://semaphoreci.com
