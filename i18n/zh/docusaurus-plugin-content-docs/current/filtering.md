...
要选择准确的包，只需指定其名称 (`@scope/pkg`) 或使用模式来选择一组包 (`@scope/*`)。
...
也可以与 `[<since>]` 合并使用。 例如，在某个目录中选择所有已更改的项目：
...
你也可以使用 [`failIfNoMatch` 设置][] 永久设置此行为。

## 排除

任何过滤规则选择器都可以作为排除项，只要在开头添加一个"!"。 在 zsh (可能还有其他 shells) 中, "!" 应被转义：`\!`。
...
[`failIfNoMatch` 设置]: workspaces.md#failifnomatch
