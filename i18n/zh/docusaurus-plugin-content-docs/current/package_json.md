...
从 v11, pnpm 不再从 `package.json` 读取 `pnpm` 设置。 设置必须改为在 `pnpm-workspace.yaml` 中定义。 请参阅 [配置](./configuring.md)。
...
2. **对于带有 `postinstall` 脚本**的软件包：该脚本使用指定的 Node.js 版本执行。
...
1. 精确的版本（和校验和）保存在锁文件（lockfile）中。
1. 脚本使用本地运行时，确保跨环境的一致性。

要覆盖声明的 `onFail` 行为而不编辑清单，请使用 [`runtimeOnFail`](./settings.md#runtimeonfail) 设置。

## devEngines.packageManager

添加于：v11.0.0

允许通过 `devEngines.packageManager` 在 `package.json` 中指定 pnpm 版本。 与 `packageManager` 字段不同，此字段支持版本范围。 已解析的版本存储在 `pnpm-lock.yaml` 中的 `packageManagerDependencies` 下，如果仍然满足范围，则会被重用。
...
当通过旧版 `packageManager` 字段（而不是 `devEngines.packageManager`）声明 pnpm 时，其解析信息 **不会** 会写入 `pnpm-lock.yaml` — 除非锁定的 pnpm 版本为 v12 或更高版本。 当从 pnpm v10 升级到 v11 时，锁文件将保持稳定，而不会强行将项目从旧版字段中移除。

:::

要覆盖 `onFail` 行为而不编辑清单，请参阅 [`pmOnFail`](./settings.md#pmonfail) 设置。
...
* [`types` 或 `typings`](https://github.com/stereobooster/package.json#types)
