...
如果你不使用独立脚本或 `@pnpm/exe` 来安装 pnpm，那么你需要在系统上安装 Node.js (至少 v22)。
...
由于这个问题，我们目前建议在 Windows 上使用 [npm](#使用-npm) 或 [Corepack](#使用-corepack) 安装 pnpm。
...
独立脚本无法在 Intel Mac 上运行（`darwin-x64`）。 请改用 [npm](#using-npm)、 [Corepack](#using-corepack)或 [Homebrew](#using-homebrew)。 请参阅 [#11423](https://github.com/pnpm/pnpm/issues/11423) 以了解上下文。
...
由于 [Corepack的签名存在过期问题](https://github.com/nodejs/corepack/issues/612)，应首先将 Corepack 更新至最新版本：

```
npm install --global corepack@latest
```

从 v16.13 开始，Node.js 提供了 [Corepack](https://nodejs.org/api/corepack.html) 来管理包管理器。 这是一项实验性功能，因此你需要通过运行如下脚本来启用它：

:::info

如果你已使用 `pnpm runtime` 安装 Node.js，Corepack 不会被安装在你的系统上，你需要单独安装它。 见 [#4029](https://github.com/pnpm/pnpm/issues/4029)。
...
- [`@pnpm/exe`](https://www.npmjs.com/package/@pnpm/exe) 与 Node.js 一起打包成可执行文件，因此它可以在没有安装 Node.js 的系统上使用。 On Linux, glibc and musl builds are both provided and the right one is selected automatically; the glibc build requires glibc 2.27 or newer and `libatomic.so.1` (see [Linux runtime requirements](#on-posix-systems) for details). **不适用于 Intel macOS** (`darwin-x64`) — 请安装 `pnpm` ，参见 [#11423](https://github.com/pnpm/pnpm/issues/11423)。
...
你想在 CI 服务器上使用 pnpm 吗？ 请参阅[持续集成](./continuous-integration.md)。
...
如果你需要从系统中删除 pnpm CLI 以及它写入磁盘的任何文件，请参阅 [卸载 pnpm][]。

[`self-update`]: ./cli/self-update.md

[卸载 pnpm]: ./uninstall.md
