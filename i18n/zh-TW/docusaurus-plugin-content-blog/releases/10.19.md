...
You can now allow specific versions of dependencies to run postinstall scripts. [`onlyBuiltDependencies`] accepts package names with exact versions (and disjunctions via `||`). 範例：
...
You can list one or more specific versions that pnpm should allow to install, even if those versions don’t satisfy the maturity requirement set by [`minimumReleaseAge`]. 範例：
