...
這是用於外寄 HTTP 要求的 Proxy。 If the HTTP_PROXY or http_proxy
...
connections to the registry. 值應採用 PEM 格式 (也就是「Base-64 編碼的 X.509 (.CER)」)。 範例：
...
PEM format (AKA "Base-64 encoded X.509 (.CER)"). 範例：
...
(AKA "Base-64 encoded X.509 (.CER)"). 範例：
