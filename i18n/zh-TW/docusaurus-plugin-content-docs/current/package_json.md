...
package.json 是套件的資訊清單檔。 它包含套件的所有中繼資料，包括相依性、標題、作者等。 This is a standard preserved across all major Node.js package managers, including pnpm.
...
附加的中繼資料，用於 `dependencies`、`optionalDependencies` 及 `devDependencies` 中宣告的依附項目 (dependency)。
