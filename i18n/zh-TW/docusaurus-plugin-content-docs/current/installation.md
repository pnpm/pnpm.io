...
由於此問題，目前我們推薦在 Windows 上透過 [npm](#using-npm) 或 [Corepack](#using-corepack) 來安裝 pnpm。
...
在執行安裝指令碼之前，您可以設定 shell 變數 `PNPM_VERSION` 以安裝特定版本的 pnpm：
...
自 16.13 版起，Node.js 推出了 [Corepack](https://nodejs.org/api/corepack.html)——管理套件管理器的工具。 因為這是試驗性功能，需執行此命令以啟用 Corepack：

:::info

If you have installed Node.js with `pnpm runtime` Corepack won't be installed on your system, you will need to install it separately. 詳情請參閱 [#4029](https://github.com/pnpm/pnpm/issues/4029)。
...
您想在 CI 伺服器上使用 pnpm 嗎？ 請參閱： [持續整合](./continuous-integration.md)。
...
首先，請嘗試執行 `which npm` 來尋找 pnpm 存放的位置。 若您使用 Windows，請執行 `where.exe pnpm.*`。 您將取得 pnpm 命令的位置，例如:
...
現在已經知道 pnpm CLI 存放的位置，請打開該資料夾並移除所有與 pnpm 有關的檔案（如 `pnpm.cmd`、`pnpx.cmd`、`pnpm` 等等）。 完成後再重新安裝 pnpm，應該就恢復正常了。
