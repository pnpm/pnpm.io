...
Una de las mejores características de pnpm es que en un proyecto, una versión específica de un paquete
siempre tendrá un conjunto de dependencias. There is one exception from
...
_If a package has no peer dependencies but has dependencies with peers that are
resolved higher in the graph_, then that transitive package can appear in the
